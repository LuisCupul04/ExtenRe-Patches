...
import static com.extenre.extension.shared.patches.PatchStatus.PatchedTime;
import static com.extenre.extension.shared.settings.BaseSettings.SPOOF_STREAMING_DATA_USE_JS;
import static com.extenre.extension.shared.utils.StringRef.str;
import static com.extenre.extension.youtube.settings.Settings.HIDE_PREVIEW_COMMENT;
import static com.extenre.extension.youtube.settings.Settings.HIDE_PREVIEW_COMMENT_TYPE;
...
import com.extenre.extension.shared.settings.preference.LogBufferManager;
import com.extenre.extension.shared.settings.preference.ToolbarPreferenceFragment;
...
import com.extenre.extension.youtube.patches.general.ChangeFormFactorPatch;
import com.extenre.extension.youtube.patches.utils.PatchStatus;
import com.extenre.extension.youtube.patches.utils.ReturnYouTubeDislikePatch;
import com.extenre.extension.youtube.returnyoutubedislike.ReturnYouTubeDislike;
import com.extenre.extension.youtube.settings.Settings;
import com.extenre.extension.youtube.settings.YouTubeHostActivity;
...
public class YouTubePreferenceFragment extends ToolbarPreferenceFragment {
    ...
    private void setBackupRestorePreference() {
        Preference importPreference = findPreference("extenre_settings_import");
        ...
        Preference exportPreference = findPreference("extenre_settings_export");
    ...
    private void setDebugLogPreference() {
        Preference clearLog = findPreference("extenre_debug_logs_clear_buffer");
        if (clearLog == null) {
            return;
        }
        ...
        Preference exportLogToClipboard = findPreference("extenre_debug_export_logs_to_clipboard");
        ...
        Preference exportLogToFile = findPreference("extenre_debug_export_logs_to_file");
    ...
    private void setPatchInformationPreference() {
        Preference appNamePreference = findPreference("extenre_app_name");
        if (appNamePreference != null) {
            appNamePreference.setSummary(ExtendedUtils.getAppLabel());
        }
        Preference appVersionPreference = findPreference("extenre_app_version");
        if (appVersionPreference != null) {
            appVersionPreference.setSummary(ExtendedUtils.getAppVersionName());
        }
        Preference patchesVersion = findPreference("extenre_patches_version");
        if (patchesVersion != null) {
            patchesVersion.setSummary(PatchVersion());
        }
        Preference patchedDatePreference = findPreference("extenre_patched_date");
    ...
    private void setSpoofStreamingDataPreference() {
        if (findPreference(Settings.SPOOF_STREAMING_DATA_DEFAULT_CLIENT.key) instanceof ListPreference listPreference
            ...
            String entriesKey = useJS
                    ? "extenre_spoof_streaming_data_default_client_with_js_entries"
                    : "extenre_spoof_streaming_data_default_client_entries";
            String entryValueKey = useJS
                    ? "extenre_spoof_streaming_data_default_client_with_js_entry_values"
                    : "extenre_spoof_streaming_data_default_client_entry_values";
