...
public final class SnackBarPatch {
    ...
    private static int getBlackColor() {
        if (blackColor == 0) blackColor = ResourceUtils.getColor("extenre_snack_bar_color_dark");
        return blackColor;
    }

    private static int getWhiteColor() {
        if (whiteColor == 0) whiteColor = ResourceUtils.getColor("extenre_snack_bar_color_light");
        return whiteColor;
    }
}
