...
import com.extenre.extension.youtube.sponsorblock.objects.UserStats;
import com.extenre.extension.youtube.sponsorblock.requests.SBRequester;
...
public class SponsorBlockStatsPreferenceCategory extends PreferenceCategory {
    ...
    private void addUserStats(Preference loadingPlaceholder, @Nullable UserStats stats) {
        ...
        try {
            ...
            if (stats.totalSegmentCountIncludingIgnored > 0) {
                ...
                preference.setTitle(fromHtml(str("extenre_sb_stats_username", userName), FROM_HTML_MODE_COMPACT));
                    ...
                    Utils.runOnBackgroundThread(() -> {
                        ...
                        Utils.runOnMainThread(() -> {
                            if (errorMessage == null) {
                                preference.setTitle(fromHtml(str("extenre_sb_stats_username", newUserName), FROM_HTML_MODE_COMPACT));
...

            {
                ...
                preference.setTitle(fromHtml(str("extenre_sb_stats_submissions", formatted), FROM_HTML_MODE_COMPACT));
...

            {
                ...
                Preference preference = new Preference(context);
                preference.setTitle(fromHtml(str("extenre_sb_stats_reputation", stats.reputation), FROM_HTML_MODE_COMPACT));
                preference.setSelectable(false);
                preference.setEnabled(Settings.SB_ENABLED.isAvailable());
...

            {
                ...
                if (stats.totalSegmentCountIncludingIgnored == 0) {
                    ...
                    stats_saved_sum = str("extenre_sb_stats_saved_sum",
    ...
    private void addLocalUserStats() {
        ...
        Runnable updateStatsSelfSaved = () -> {
            ...
            preference.setTitle(fromHtml(str("extenre_sb_stats_self_saved", formatted), FROM_HTML_MODE_COMPACT));

            String formattedSaved = SponsorBlockUtils.getTimeSavedString(
                    Settings.SB_LOCAL_TIME_SAVED_MILLISECONDS.get() / 1000);
            preference.setSummary(fromHtml(str("extenre_sb_stats_self_saved_sum", formattedSaved), FROM_HTML_MODE_COMPACT));
        };
        ...
        preference.setOnPreferenceClickListener(preference1 -> {
            Pair<Dialog, LinearLayout> dialogPair = CustomDialog.create(
                    ...
                    str("extenre_sb_stats_self_saved_reset_title"), // Title.
                    null, // No message.
                    null, // No EditText.
                    null, // OK button text.
                    () -> {
