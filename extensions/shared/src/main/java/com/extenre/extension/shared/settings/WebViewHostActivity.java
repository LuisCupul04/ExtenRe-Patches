...
import static com.extenre.extension.shared.utils.ResourceUtils.getLayoutIdentifier;
...
public class WebViewHostActivity extends Activity {
    ...
    protected void onCreate(Bundle bundle) {
        ...
        try {
            ...
            if (!StringUtils.equals("extenre_webview", dataString)) {
                Logger.printDebug(() -> "onCreate ignored");
                return;
            }
            if (!Utils.isNetworkConnected()) {
                Utils.showToastShort(str("extenre_webview_toast_no_network"));
                return;
            }
            if (!Utils.isWebViewSupported()) {
                Utils.showToastShort(str("extenre_webview_toast_unavailable"));
                return;
            }
            ...
            setContentView(getLayoutIdentifier("extenre_webview"));
            ...
            webView = findViewById(getIdIdentifier("extenre_webview"));
    ...
    protected void setToolbar() {
        ViewGroup toolBarParent = findViewById(getIdIdentifier("extenre_toolbar_parent"));

        // Remove dummy toolbar.
        ViewGroup dummyToolbar = toolBarParent.findViewById(getIdIdentifier("extenre_toolbar"));
        ...
        if (BaseThemeUtils.isSupportModernDialog) {
            toolbar.inflateMenu(getMenuIdentifier("extenre_webview_menu"));
            ...
            int getCookies = getIdIdentifier("extenre_webview_get_cookies");
            int getDataSyncId = getIdIdentifier("extenre_webview_get_data_sync_id");
            int getVisitorData = getIdIdentifier("extenre_webview_get_visitor_data");
            int refresh = getIdIdentifier("extenre_webview_refresh");
            ...
            toolbar.setOnMenuItemClickListener(item -> {
                try {
                    ...
                    if (itemId == getCookies) {
                        showDialog(cookies, str("extenre_webview_cookies"));
                        return true;
                    } else if (itemId == getDataSyncId) {
                        showDialog(dataSyncId, str("extenre_webview_data_sync_id"));
                        return true;
                    } else if (itemId == getVisitorData) {
                        showDialog(visitorData, str("extenre_webview_visitor_data"));
                        return true;
    ...
    private void showDialog(String content, String title) {
        if (StringUtils.isEmpty(content)) {
            Utils.showToastShort(str("extenre_webview_toast_empty", title));
            return;
        }
        ...
        Pair<Dialog, LinearLayout> dialogPair = CustomDialog.create(
                ...
                null,
                // OK button text.
                str("extenre_webview_copy"),
                // OK button action.
                () -> {
                    Utils.setClipboard(content);
                    Utils.showToastShort(str("extenre_webview_toast_copy_success", title));
                },
