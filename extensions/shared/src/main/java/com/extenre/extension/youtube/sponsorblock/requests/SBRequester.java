...
package com.extenre.extension.youtube.sponsorblock.requests;
...
import com.extenre.extension.shared.sponsorblock.requests.SBRoutes;
...
import com.extenre.extension.youtube.sponsorblock.objects.SponsorSegment.SegmentVote;
...
public class SBRequester {
    ...
    public static void submitSegments(String videoId, String category,
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                Utils.showToastLong(str("extenre_sb_submit_succeeded"));
                return;
            }

            String userErrorMessage = switch (responseCode) {
                case 409 -> str("extenre_sb_submit_failed_duplicate");
                case 403 -> str("extenre_sb_submit_failed_forbidden",
                        Requester.parseErrorStringAndDisconnect(connection));
                case 429 -> str("extenre_sb_submit_failed_rate_limit");
                case 400 -> str("extenre_sb_submit_failed_invalid",
                        Requester.parseErrorStringAndDisconnect(connection));
                default -> str("extenre_sb_submit_failed_unknown_error",
            ...
            try {
                ...
                switch (responseCode) {
                    case HTTP_STATUS_CODE_SUCCESS:
                        ...
                        return;
                    case 403:
                        userMessage = str("extenre_sb_vote_failed_forbidden",
                                Requester.parseErrorStringAndDisconnect(connection));
                        break;
                    default:
                        userMessage = str("extenre_sb_vote_failed_unknown_error",
                                responseCode, connection.getResponseMessage());
                        break;
                }
